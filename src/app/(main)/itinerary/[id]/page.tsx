import { ArrowLeft, Map, Share2, Star, Navigation } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen pb-24 pt-8">
      <div className="container-app max-w-3xl">
        ...
        {/* Bản đồ lộ trình toàn cảnh (M14 Upgrade) */}
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <div className="mt-2 text-slate-500 font-medium">
                Khởi hành: {new Date(itinerary.date).toLocaleDateString('vi-VN')}
              </div>
            </div>
            <div className="flex flex-row gap-4 sm:flex-col sm:items-end shrink-0">
              <div className="flex flex-col items-start sm:items-end">
                <span className="text-xs font-semibold text-slate-400 uppercase">Tổng chi phí</span>
                <span className="font-bold text-lg text-primary-600 dark:text-primary-400">{formatCurrency(itinerary.totalBudget)}</span>
              </div>
              <div className="h-8 w-px bg-slate-200 dark:bg-slate-800 sm:hidden" />
              <div className="flex flex-col items-start sm:items-end">
                <span className="text-xs font-semibold text-slate-400 uppercase">Quãng đường</span>
                <span className="font-bold text-lg text-slate-900 dark:text-white">{itinerary.totalDistance} km</span>
              </div>
            </div>
          </div>
        ...
        {/* Floating Action Button (M14 Upgrade) */}
